...
module.exports.addCoupons = async function (req, res) {
  ...
  try{
    ...
    if(!(req.body.item.couponString)) {
        return responseData(res, false, 200, 'coupon invalid', req.body.item.couponString);
      }
  
    let isCouponValid =  await checkCouponCode(req.body.item.couponString.toUpperCase());
    if(!(isCouponValid && isCouponValid.length >0 )) {
      return responseData(res, false, 200, 'Coupon invalid', req.body.item.couponString)
    }
  
    let isCodeEnds =  await checkCouponTime(req.body.item.couponString.toUpperCase() , moment().format('YYYY-MM-DD'));
    ...
    if( isCouponValid[0].isApplyOnes) {
      ...
      let isFirstTimeValid =  await checkFirstTime(req.body.item.userCookie , isCouponValid[0]._id );
      if(!(isFirstTimeValid)) {
        return responseData(res, false, 200, 'Coupon invalid', req.body.item.couponString)
      }
    ...
    if(req.body.cartCookie) {
      ...
      let discountPrice;
      let userCart = await db.collection("cart").find({id:req.body.cartCookie}).toArray();
  
      if(isCouponValid[0]?.Discount?.type == "Price") {
        discountPrice = userCart[0].subtotalPrice - isCouponValid[0]?.Discount.totalDiscount;
      }
      else{
        discountPrice = userCart[0].subtotalPrice - (userCart[0].subtotalPrice * isCouponValid[0]?.Discount.totalDiscount)%100;
      }
    ...
    return responseData(res, true, 200, 'Coupon added successfully', req.body.item.couponString)
  }
  catch(err) {
    console.log(err);
    return responseData(res, false, 200, 'Error while applying coupon', req.body.item.couponString)
  }
...
async function checkCouponCode(code) {
  try{
    const db = getDb();
    return  await db.collection("coupons").find({ code : code , isActive: true }).toArray();
...
async function checkCouponTime(code ,date) {
  try{
    ...
    const db = getDb();
    let coupon =  await db.collection("coupons").find({ code :code , isActive: true}, {end : 1}).toArray();
...
async function checkFirstTime(userCookie , coupanId) {
  ...
  try{
    const db = getDb();
    let isUserExist =  await db.collection("anonymousUser").find({ userId :userCookie }).toArray();

    if(isUserExist && isUserExist.length>0) {

       let isCouponAlreadyExist = await db.collection("anonymousUser").find({ userId :userCookie } , { coupon : 1}).toArray();

       if(isCouponAlreadyExist && isCouponAlreadyExist > 0 ) {

        let coupon =  await db.collection("coupons").find({ _id : ObjectId(coupanId)}).toArray();
        ...
        isCouponAlreadyExist[0].coupon.forEach(element => {
          ...
          await db.collection("anonymousUser").updateOne({ userId : userCookie },
            {
              $push : { 
                coupon: coupanId
            }});
